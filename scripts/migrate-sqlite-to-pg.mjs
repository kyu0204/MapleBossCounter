...
async function main() {
  ...
  for (const [table, bools, jsons, hasSeq] of TABLES) {
    ...
    const [{ count }] = await sql`select count(*)::int as count from ${sql(bare(table))}`;
    if (count > 0) {
    ...
    // 한 번에 다 넣으면 파라미터 한도에 걸린다. 200행씩 끊는다.
    for (let i = 0; i < converted.length; i += 200) {
      await sql`insert into ${sql(bare(table))} ${sql(converted.slice(i, i + 200))}`;
    }
    ...
    if (hasSeq) {
      ...
      await sql.unsafe(
        `select setval(pg_get_serial_sequence('${bare(table)}', 'id'), coalesce((select max(id) from ${bare(table)}), 1))`,
      );
    }
  }
