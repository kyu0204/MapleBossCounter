...
export default async function MePage({ searchParams }: PageProps<"/me">) {
  ...
  const cards = visible.map((c) => {
    ...
    const rev = picks.length
      ? { total: picksTotals(picks, priceDate).value, earned: picksTotals(done, priceDate).value, remainingCount: remaining.length }
      : null;
    return { c, snap, rev };
  });
  ...
  const grand = cards.reduce(
    (s, x) => ({ total: s.total + (x.rev?.total ?? 0), earned: s.earned + (x.rev?.earned ?? 0), remaining: s.remaining + (x.rev?.remainingCount ?? 0) }),
